...
# Configurações do Supabase via central config
supabase_url = settings.SUPABASE_URL
supabase_admin_key = settings.SUPABASE_SERVICE_ROLE_KEY
...
def invite_member(
    ...
    try:
        ...
        # 3. Gerar Link via settings
        invite_link = f"{settings.FRONTEND_URL}/finalizar-registro?token={token}"
        ...
        def process_email_sending():
            try:
                if settings.ENVIRONMENT == "development" or settings.DEV_MODE:
                    logger.info(
...
def resend_invite(
    ...
    # Reenviar com URL centralizada
    invite_link = f"{settings.FRONTEND_URL}/finalizar-registro?token={invite.token}"
